...
def mouse_callback(event, u, v, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and len(pixel_points) < NUM_POINTS:
        ...
        print(f"{len(pixel_points)}: (u={u}, v={v})")
...
def run_calibration_tool():
    ...
    print(f"1. click theo thứ tự (P1 -> P4) vào 4 điểm tham chiếu cố định.")
    ...
    if len(pixel_points) == NUM_POINTS:    
        ...
        try:
            ...
            print(f"\nMa trận H đã lưu '{OUTPUT_FILE}'")

            
        except Exception as e:
            print(f"LỖI TÍNH TOÁN {e}")
            print("   Kiểm tra lai 4 điểm")
    
    else:
        print("Không tính toán duoc Homography")
