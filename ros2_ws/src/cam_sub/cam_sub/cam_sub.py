...
class ConveyorVisionProcessor(Node):
    def __init__(self, name):
        ...
        # 3. Trang thai dieu khien
        self.arm_free = True        
        self.waiting_for_robot = False 
        
        # xu ly moi N khung hinh de giam tai CPU
        self.frame_count = 0
        self.process_every_n_frames = 2 # Chi chay moi 2 khung hinh (tang FPS)

        # 4. ROS setup
        self.image_sub = self.create_subscription(
            CompressedImage, 'image_raw/compressed', self.listener_callback, 
            qos_profile_sensor_data) #  Giu qos_profile_sensor_data  de giam tre
        ...
        self.get_logger().info(" Tối ưu chống lag khởi chạy...")
    ...
    def listener_callback(self, data):
        ...
        if self.frame_count % self.process_every_n_frames != 0:
            return # Bo qua khung hinh nay de CPU nghi ngoi 
        ...
        if len(results[0].boxes) > 0:
            ...
            for box, cls in zip(boxes, clss):
                # Lưu ý: Do đã resize ảnh xuống 1/2 nen nhân tọa độ tâm lên 2
